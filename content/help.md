...
SmokySignal tracks the law-enforcement aircraft over Washington. Anything in the air with an enforcement mission reads as "Bird" — that's the umbrella. Underneath, we keep four backend roles so plane-detail pages can tell you exactly what airframe is up.

**BIRD (speed enforcement)** — fixed-wing planes, usually Cessnas with FLIR cameras, used for clocking speed from the air. WSP runs five of these. Pierce SO runs one. Pill: BIRD UP. Headline: "Eye In The Sky."

**BIRD (patrol helicopter)** — county sheriff helicopters that may be doing traffic, pursuit, or SAR. Same BIRD UP pill. Body copy still says "patrol helicopter" so you know the airframe.
...
Display preferences live on your device. Push alert settings, including selected region and radius, sync to your notification subscription so closed-app alerts can work. The home for these toggles is [Settings → Alerts](/settings/alerts).
...
**Region** — Pick your state and region. The selected region drives the radar scope and the center ZIP used for background proximity alerts. Also adjustable from the radar top-bar dropdown.
...
**Proximity ping** — When armed, sends an alert any time an alert-tier tail comes within your chosen radius of your selected region center ZIP. Push-capable browsers can receive these alerts even when the website is closed.
...
When a Bird enters the range set for your selected region, you get a push, even with the tab closed. Quiet hours (22:00–06:00 local) suppress non-critical pushes by default; emergency squawks always come through.
...
- **BIRD UP / Eye In The Sky.** — A law-enforcement plane is in the air — fixed-wing FLIR or patrol helicopter. Mind the throttle.
- **ALL CLEAR / No Eyes.** — Nothing alert-class is up. If a SAR or transport aircraft happens to be in the air, you'll see a small footnote.
...
- **BIRD UP** (amber) — a law-enforcement plane is up (fixed-wing FLIR, patrol helicopter, or unconfirmed alert-class)
...
The bottom-left map controls toggle distance rings and current flight paths. Distance rings need location permission; flight paths show recent aircraft movement while a tail is active.
...
The Recent events section in Dash shows state-change events for tracked tails:
...
Tap a row to open the plane detail page. The feed stays current while Dash is visible.

### Plane detail

Each tail has its own page at `/plane/{TAIL}`. It shows:

- **Status pill**: `AIRBORNE · WATCHING` (amber) or `GROUNDED` (green) with last-seen relative time, plus a small role badge ("BIRD" for any law-enforcement plane, "SEARCH & RESCUE", or "TRANSPORT")
...
Your wake-lock preference, region, time-format, contrast, voice and proximity toggles, and custom zones all live on your phone's local storage or in non-tracking cookies, and never leave the device. Push subscriptions are the one exception — Web Push requires your browser endpoint server-side. We treat it as PII-equivalent and use it only to send the alerts you asked for.
...
A month is the floor for the forecast features to mean something:
...
**Region** — A state/region preset backed by a center ZIP code. Drives radar centering and background proximity alert matching.
